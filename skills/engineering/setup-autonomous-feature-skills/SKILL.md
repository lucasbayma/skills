...
description: Configure a repo for Candango delivery skills. Use before feature-plan, feature-spec, feature-issues, feature-uat, feature-uat-runner, autonomous-feature-executor, or feature-wrap-up when the repo has not yet declared its issue tracker, feature folder layout, UAT location, validation command discovery rules, dashboard location, design artifact location, or agent execution conventions.
---
...
Candango role: setup layer. Prepares a repo so the full Candango workflow can run with known docs, tracker, validation, design, UAT, and execution rules.
