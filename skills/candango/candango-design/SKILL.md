...
description: Candango Design. Create and review high-fidelity UI screens, web/app prototypes, interaction demos, design variants, HTML visual artifacts, slides, motion demos, and design direction explorations. Use when the user asks for UI mockups, prototype, app screens, web screens, Figma/design work, visual layout, design variants, HTML demo, or when candango-discover identifies feature business scope that includes an interface. Before creating/editing UI for a feature, ask whether to create or edit layouts/screens and where the work should live. All user-facing communication and reports must use candango-caveman style. Fork credit to Huashu Design and the local huashu-design skill.
---
...
Use repo-local `$candango-caveman` for every user-facing question, status update, and report.

When `$candango-discover` resolves business definitions and feature scope includes interface work, run this skill before implementation planning/spec finalization.

Interface scope includes:

- web page or web app screen
- mobile app screen
- dashboard/admin panel
- onboarding, checkout, settings, forms, tables
- user flow, prototype, layout, visual design
- design system/component changes

Before creating or editing screens, ask one caveman-style question:
...
If user says no, record `Design artifacts skipped by user` in feature plan/spec.

If user says yes:

- capture target surface: Figma/local HTML/codebase/etc.
- ask for existing design system/UI kit/screenshots/brand assets
- produce or update design artifacts before issues are finalized
- link artifacts from `candango-plan`, `candango-spec`, and related issues
- create UAT/design acceptance signals for screen behavior and visual states
- save local HTML/design artifacts under `docs/features/<feature-slug>/design/`
...
| **Gallery Ripple + Multi-Focus 场景哲学**（当素材 20+ 同质+场景需表达「规模×深度」时优先用；含前置条件、技术配方、5 个可复用模式）| `references/hero-animation-case-study.md`（candango-design hero v9 蒸馏）|
