---
name: huashu-design
description: Create and review high-fidelity UI screens, web/app prototypes, interaction demos, design variants, HTML visual artifacts, slides, motion demos, and design direction explorations. Use when the user asks for UI mockups, prototype, app screens, web screens, Figma/design work, visual layout, design variants, HTML demo, or when grill-with-docs identifies feature business scope that includes an interface (web, mobile app, dashboard, admin panel, onboarding, checkout, settings, forms, flows, or any user-facing screen). Before creating/editing UI for a feature, ask the user whether they want layout/screens created or edited and where the work should live (Figma, local HTML prototype, existing codebase, screenshots, or other design surface). All user-facing communication and reports must use caveman style.
---
...
When `$grill-with-docs` resolves business definitions and feature scope includes interface work, run this skill before implementation planning/spec finalization.
...
- link artifacts from `feature-plan`, `feature-spec`, and related issues
